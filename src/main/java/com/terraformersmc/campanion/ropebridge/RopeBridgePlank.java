...
public class RopeBridgePlank {
	...
	public RopeBridgePlank(BlockPos from, BlockPos to, Vec3d deltaPosition, double yAngle, double tiltAngle,
			float distToPrevious, float ropesSubtract, int ropeVariant, boolean flat, boolean master, boolean ropes,
			boolean stopper) {
		this.from = from;
		this.to = to;
	...
	public static RopeBridgePlank deserialize(CompoundTag tag) {
		RopeBridgePlank plank = new RopeBridgePlank(BlockPos.fromLong(tag.getLong("FromPos")),
				BlockPos.fromLong(tag.getLong("ToPos")),
				new Vec3d(tag.getDouble("DeltaPosX"), tag.getDouble("DeltaPosY"), tag.getDouble("DeltaPosZ")),
				tag.getFloat("YAng"), tag.getFloat("TiltAng"), tag.getFloat("DistToPrevious"),
				tag.getFloat("RopesSubtract"), tag.getByte("RopeVariant"), tag.getBoolean("Flat"),
				tag.getBoolean("Master"), tag.getBoolean("Ropes"), tag.getBoolean("Stopper"));
	...
	public void generateMesh(boolean translucent, QuadEmitter emitter) {
		...
		RenderMaterial material = RendererAccess.INSTANCE.getRenderer().materialFinder().blendMode(0, BlendMode.CUTOUT)
				.find();
	...
	private void renderPlank(QuadEmitter emitter, MatrixStack stack, RenderMaterial material, Random random,
			boolean translucent) {
		stack.push();
		if (!this.flat) {
	...
	private void drawRope(QuadEmitter emitter, MatrixStack stack, RenderMaterial material, Random random,
			boolean translucent) {
		...
		for (int modifier : new int[] { -1, 1 }) {
			...
			stack.push();
			stack.translate(0, RopeBridge.ROPE_LENGTH / 16F - this.ropesSubtract,
					modifier * RopeBridge.PLANK_LENGTH / 2D);
			if (this.ropes) {
				// Rotate it a tiny bit as to not make the knots axis aligned. See:
				// https://imgur.com/iCGrpn9
				stack.push();
	...
	private void drawBottomRopes(QuadEmitter emitter, MatrixStack stack, RenderMaterial material, Sprite rope,
			Random random, boolean translucent, int modifier) {
		stack.push();
		stack.translate(0, -0.025,
				modifier * (RopeBridge.PLANK_LENGTH / 2F - RopeBridge.UNDER_ROPE_DIST_FROM_EDGE / 16F));
		stack.multiply(Vector3f.POSITIVE_Z.getRadialQuaternion(this.tiltAngle));
		...
		this.drawDoubleSided(emitter, material,
				...
				(v, m) -> add(emitter, v, stack, translucent, 0, 0, modifier * -RopeBridge.ROPE_WIDTH / 16F, minU, minV,
						0, -m * modifier, 0),
				(v, m) -> add(emitter, v, stack, translucent, this.distToPrevious, 0,
						modifier * -RopeBridge.ROPE_WIDTH / 16F, minU, maxV, 0, -m * modifier, 0),
				(v, m) -> add(emitter, v, stack, translucent, this.distToPrevious, 0, 0, maxU, maxV, 0, -m * modifier,
						0));

		stack.pop();
	}

	private void drawConnectingRope(QuadEmitter emitter, MatrixStack stack, RenderMaterial material, Sprite rope,
			Random random, boolean translucent, int modifier) {
		float ropeStart = random.nextFloat() * (16F - this.distToPrevious * 16F);
		...
		if (this.distToPrevious != 0) {
			this.drawDoubleSided(emitter, material,
					(v, m) -> add(emitter, v, stack, translucent, 0, -RopeBridge.ROPE_WIDTH / 32F, 0, ropeMinU,
							ropeMinV, 0, 0, modifier * m),
					(v, m) -> add(emitter, v, stack, translucent, this.distToPrevious, -RopeBridge.ROPE_WIDTH / 32F, 0,
							ropeMinU, ropeMaxV, 0, 0, modifier * m),
					(v, m) -> add(emitter, v, stack, translucent, this.distToPrevious, RopeBridge.ROPE_WIDTH / 32F, 0,
							ropeMaxU, ropeMaxV, 0, 0, modifier * m),
					(v, m) -> add(emitter, v, stack, translucent, 0, RopeBridge.ROPE_WIDTH / 32F, 0, ropeMaxU, ropeMinV,
							0, 0, modifier * m));
		}
	}

	private void drawVerticalRope(QuadEmitter emitter, MatrixStack stack, RenderMaterial material, Sprite rope,
			Random random, boolean translucent, int modifier) {
		float hl = (float) (RopeBridge.PLANK_LENGTH / 2D); // half length
		...
		this.drawDoubleSided(emitter, material,
				(v, m) -> add(emitter, v, stack, translucent, -RopeBridge.ROPE_WIDTH / 32F,
						RopeBridge.ROPE_LENGTH / 16F - this.ropesSubtract, mhl, ropeMinU, ropeMinV, 0, 0, m * modifier),
				(v, m) -> add(emitter, v, stack, translucent, -RopeBridge.ROPE_WIDTH / 32F, 0, mhl, ropeMinU, ropeMaxV,
						0, 0, m * modifier),
				(v, m) -> add(emitter, v, stack, translucent, RopeBridge.ROPE_WIDTH / 32F, 0, mhl, ropeMaxU, ropeMaxV,
						0, 0, m * modifier),
				(v, m) -> add(emitter, v, stack, translucent, RopeBridge.ROPE_WIDTH / 32F,
						RopeBridge.ROPE_LENGTH / 16F - this.ropesSubtract, mhl, ropeMaxU, ropeMinV, 0, 0,
						m * modifier));
	}

	private void drawKnot(QuadEmitter emitter, MatrixStack stack, RenderMaterial material, Sprite rope, Random random,
			boolean translucent) {
		int ropeKnotUStart = random.nextInt(16 - RopeBridge.KNOT_SIZE);
		int ropeKnotVStart = random.nextInt(16 - RopeBridge.KNOT_SIZE);
		drawCube(emitter, material,
				(index, x, y, z, u, v, nx, ny, nz, dir) -> add(emitter, index, stack, translucent,
						x * RopeBridge.KNOT_SIZE / 32F, y * RopeBridge.KNOT_SIZE / 32F, z * RopeBridge.KNOT_SIZE / 32F,
						rope.getFrameU(ropeKnotUStart + RopeBridge.KNOT_SIZE * Math.signum(u + 1F)),
						rope.getFrameV(ropeKnotVStart + RopeBridge.KNOT_SIZE * Math.signum(v + 1F)), nx, ny, nz));
	}

	public static void drawStopper(QuadEmitter emitter, MatrixStack stack, RenderMaterial material, Random random,
			boolean translucent, int modifier) {
		stack.push();
		...
		drawCube(emitter, material, (index, x, y, z, u, v, nx, ny, nz, dir) -> {
			...
			add(emitter, index, stack, translucent, x * RopeBridge.STOPPER_WIDTH / 32F,
					zo(y) * RopeBridge.STOPPER_HEIGHT / 16F, z * RopeBridge.STOPPER_WIDTH / 32F, texU, texV, nx, ny,
					nz);
		});
		stack.pop();
	}
	...
	private static void drawCube(QuadEmitter emitter, RenderMaterial material, CubeRenderCallback callback) {
		...
		for (Direction d : Direction.values()) {
			...
			for (int v = 0; v < 4; v++) {
				...
				boolean flip = (d.getDirection() == Direction.AxisDirection.POSITIVE) != (d
						.getAxis() == Direction.Axis.X);
				callback.apply(flip ? v : 3 - v, x[v], y[v], z[v], zo(vertex2[v]), zo(vertex1[v]), d.getOffsetX(),
						d.getOffsetY(), d.getOffsetZ(), d);
			}
			emitter.emit();
		}
	}
	...
	private void drawDoubleSided(QuadEmitter emitter, RenderMaterial material, VertexConsumer v1, VertexConsumer v2,
			VertexConsumer v3, VertexConsumer v4) {
		emitter.material(material);
		v1.apply(0, 1);
		v2.apply(1, 1);
		v3.apply(2, 1);
	...
	private static void add(QuadEmitter emitter, int index, MatrixStack stack, boolean translucent, float x, float y,
		...
		emitter.pos(index, pos.getX(), pos.getY(), pos.getZ()).sprite(index, 0, u, v)
				.spriteColor(index, 0, translucent ? 0x88FFFFFF : 0xFFFFFFFF).cullFace(null).normal(index, normal);
	}
