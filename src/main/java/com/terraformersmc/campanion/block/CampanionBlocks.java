...
public class CampanionBlocks {
	...
	public static final Block ROPE_BRIDGE_POST = add(
			"rope_bridge_post", new RopeBridgePostBlock(FabricBlockSettings.of(Material.WOOD)
					.sounds(BlockSoundGroup.WOOD).strength(1.0F, 1.0F).dynamicBounds().nonOpaque().build()),
			ItemGroup.TOOLS);
	public static final Block ROPE_BRIDGE_PLANKS = add("rope_bridge_planks",
			new RopeBridgePlanksBlock(FabricBlockSettings.of(Material.WOOD).sounds(BlockSoundGroup.WOOD)
					.strength(0.5F, 1.0F).dynamicBounds().nonOpaque().dropsNothing().build()));
	public static final Block ROPE_LADDER = add("rope_ladder", new RopeLadderBlock(FabricBlockSettings.of(Material.WOOD)
			.nonOpaque().hardness(0.2F).sounds(BlockSoundGroup.LADDER).dropsNothing().build()), ItemGroup.DECORATIONS);
	...
	public static final Block LEATHER_TANNER = add("leather_tanner",
			new LeatherTanner(FabricBlockSettings.of(Material.WOOD).nonOpaque().sounds(BlockSoundGroup.WOOD).build()),
	...
	public static final Block TENT_POLE = add("tent_pole", new TentPoleBlock(
			FabricBlockSettings.of(Material.WOOD).nonOpaque().hardness(-1F).sounds(BlockSoundGroup.WOOD).build()),
			(ItemGroup) null);
	...
	private static TentSideBlock tentSide(DyeColor color) {
		return add(color.getName() + "_tent_side", new TentSideBlock(
				FabricBlockSettings.of(Material.WOOL).nonOpaque().hardness(1F).sounds(BlockSoundGroup.WOOL).build(),
				color), (ItemGroup) null);
	}

	private static TentTopBlock tentTop(DyeColor color) {
		return add(color.getName() + "_tent_top", new TentTopBlock(
				FabricBlockSettings.of(Material.WOOL).nonOpaque().hardness(1F).sounds(BlockSoundGroup.WOOL).build(),
				color), (ItemGroup) null);
	}

	private static TentTopPoleBlock toppedTentPole(DyeColor color) {
		return add(color.getName() + "_topped_tent_pole", new TentTopPoleBlock(
				FabricBlockSettings.of(Material.WOOL).nonOpaque().hardness(1F).sounds(BlockSoundGroup.WOOL).build(),
				color), (ItemGroup) null);
	}

	private static TentTopFlatBlock tentTopFlat(DyeColor color) {
		return add(color.getName() + "_flat_tent_top", new TentTopFlatBlock(
				FabricBlockSettings.of(Material.WOOL).nonOpaque().hardness(1F).sounds(BlockSoundGroup.WOOL).build(),
				color), (ItemGroup) null);
	}

	private static LawnChairBlock createLawnChair(String color) {
		return add(color + "_lawn_chair",
				new LawnChairBlock(
						FabricBlockSettings.of(Material.WOOD).nonOpaque().sounds(BlockSoundGroup.WOOD).build()),
				ItemGroup.DECORATIONS);
	}
